...
const DashboardLayout: React.FC<{ account: string }> = ({ account }) => {
  ...
  if (!context) {
    throw new Error(
      'DashboardLayout must be inside of the provider UserContext',
    )
  }
  ...
  const [activeView, setActiveView] = useState<
    'groupDetails' | 'transactions' | 'friendDetails' | null
  >(null)
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false)

  const fetchGroups = async (userAddress: string) => {
    const response = await axios.get(
      `http://localhost:3001/api/groups/getUserGroups/${userAddress}`,
    )
      ...
      <div className="content-section">
        ...
        {!isLoadingGroups && !isErrorGroups && (
          <>
            ...
            {activeView === 'friendDetails' && selectedFriend && (
              <FriendDetails friend={selectedFriend} />
            )}
          </>
        )}
      </div>
